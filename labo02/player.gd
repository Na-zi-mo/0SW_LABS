...
var shoot_interval = 1.0 / 4.0  # 4 balles par seconde
...
@onready var bullet_pool_scene : PackedScene = preload("res://bullet_pool.tscn")
@onready var bullet_pool_instance : BulletPool

func _ready():
	bullet_pool_instance = bullet_pool_scene.instantiate()
	add_child(bullet_pool_instance) 
	spawn()
...
func shoot():
	var b = bullet_pool_instance.get_bullet()
	if b != null:
		if b.get_parent() == bullet_pool_instance:
			bullet_pool_instance.remove_child(b)
